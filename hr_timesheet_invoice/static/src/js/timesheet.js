odoo.define('hr_timesheet_invoice', function (require) {
    "use strict";
    ...
    setTimeout(function () {
        ...
        var CustomerWeeklyTimesheet = WeeklyTimesheet.extend({
                ...
                return this.render_drop.add(new Model('account.analytic.line').call('default_get', [
                    ...
                    accounts = _.chain(self.get('sheets'))
                        ...
                        .each(function (el) {
                            ...
                            el['id_group'] = el.account_id +'-'+ el.code7_id;

                        })
                        .groupBy('account_id').value();
                    var account_ids = _.map(_.keys(accounts), function (el) {
                        return el === 'false' ? false : Number(el);
                    });
                    var templateAccounts = {};
                    var test = _.map(_.keys(accounts), function (el) {
                        var linesDay = accounts[el];
                        var groupedDays = _.groupBy(linesDay, function (data) {
                            return data.id_group;
                        });
                        templateAccounts[el] = groupedDays;
                    });
                    var listAccounts = [];
                    var intialAccounts = _(templateAccounts).chain().map(function (array_lines, account_id) {

                        // group by days
                        account_id = (account_id === "false") ? false : Number(account_id);
                        $.each(array_lines, function (i, lines) {
                            var accounts_defaults = _.extend({}, default_get, (lines|| {}).value || {});
                            console.log(accounts_defaults);
                            ...
                            listAccounts.push({
                                ...
                                days: days,
                                code7_id: code7_id,
                                non_code_activity: non_code_activity,
...
//                    // we need the name_get of the accounts
            ...
            display_totals: function () {
                ...
                _.each(self.accounts, function (account) {
                    ...
                    self.$('[data-project-total="' + account.account_id + '"][data-code7-total="' + account.code7_id + '"]').html(self.format_client(acc_tot));
                });
                _.each(_.range(self.dates.length), function (day_count) {
        ...
        core.form_custom_registry.add('weekly_timesheet', CustomerWeeklyTimesheet);
    }, 100);
});
